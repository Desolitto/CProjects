...
long long int my_str_to_num(const char *s, int *n_num, flagss flags,
  ...
  if (flags.width > 0 && !*isWrongWidth) {
    for (n = 0; my_isdigit(s[i]) && (flags.width > 0); i++, flags.width--) {
      n = base * n + (s[i] - '0');
    }
  } else {
...
double my_f_str_to_num(const char *s, int *n_num, int *isDigitExist,
  ...
  if (my_isInfNan(s, &i, &n))
    ...
    double decimal = 10.0;
    if (flags.width > 0 && !*isWrongWidth) {
      while ((my_isdigit(s[i]) || s[i] == '.') && (flags.width > 0)) {
        *isDigitExist = 1;
        if (s[i] == '.') {
          i++;
          break;
      ...
      while (my_isdigit(s[i]) && (flags.width > 0)) {
        n = n + (s[i++] - '0') / decimal;
        decimal *= 10;
        flags.width--;
      }
    } else {
      while (my_isdigit(s[i]) || s[i] == '.') {
        *isDigitExist = 1;
        if (s[i] == '.') {
          i++;
          break;
...
unsigned long long int my_xXp_str_to_num(const char *s, int *n_num,
  ...
  if (flag != 1) {
    if (flags.width > 0 && !*isWrongWidth) {
      for (n = 0; s[i] != '\0' &&
                  (my_isdigit(s[i]) ||
                   (my_strchr("abcdefABCDEF", s[i]) != my_NULL)) &&
                  (flags.width > 0);
           i++, flags.width--) {
        my_xX_to_num(s[i], &n, base);
      }
    } else {
      int count = 0;
      for (n = 0;
           s[i] != '\0' && (my_isdigit(s[i]) ||
                            (my_strchr("abcdefABCDEF", s[i]) != my_NULL));
           i++) {
        my_xX_to_num(s[i], &n, base);
        count++;
...
int my_handleSpacesInSpecifiers(const char **format, int *lengthStr) {
  ...
  while (*lengthStr > 0 && my_isspace(**format)) {
    isSpaceinfromat = 1;
    (*format)++;
    (*lengthStr)--;
  }
...
int my_handleCSpec(const char **str, va_list *tr, flagss *flags,
  ...
  if (!flags->readAndSkip) {
    my_strncpy(va_arg(*tr, char *), *str, 1);
    count++;
    (*read_from_str)++;
  }
...
int my_handleDSpec(int *resWidth, const char **str, va_list *tr, flagss flags,
  ...
  if (flags.length_mode == 1) {
    short_num = my_str_to_num(*str, &shift, flags, &isWrongWidth, 10);
  } else if (flags.length_mode == 2) {
    long_num = my_str_to_num(*str, &shift, flags, &isWrongWidth, 10);
  } else {
    num = my_str_to_num(*str, &shift, flags, &isWrongWidth, 10);
  }
...
int my_handleUOSpec(int *resWidth, const char **str, va_list *tr, flagss flags,
  ...
  if (flags.length_mode == 1) {
    short_num = my_str_to_num(*str, &shift, flags, &isWrongWidth, base);
  } else if (flags.length_mode == 2) {
    long_num = my_str_to_num(*str, &shift, flags, &isWrongWidth, base);
  } else {
    num = my_str_to_num(*str, &shift, flags, &isWrongWidth, base);
  }
...
int my_handleXSpec(int *resWidth, const char **str, va_list *tr, flagss flags,
  ...
  if (flags.length_mode == 1) {
    short_num = my_xXp_str_to_num(*str, &shift, flags, &isWrongWidth, 16);
  } else if (flags.length_mode == 2) {
    long_num = my_xXp_str_to_num(*str, &shift, flags, &isWrongWidth, 16);
  } else {
    num = my_xXp_str_to_num(*str, &shift, flags, &isWrongWidth, 16);
  }
...
int my_handlePSpec(int *resWidth, const char **str, va_list *tr, flagss flags,
  ...
  num_p = my_xXp_str_to_num(*str, &shift, flags, &isWrongWidth, 16);
...
int my_handleFSpec(int *resWidth, const char **str, va_list *tr, flagss flags,
  ...
  if (flags.length_mode == 3) {
    long_double_num =
        my_f_str_to_num(*str, &shift, &isDigitExist, flags, &isWrongWidth);
  } else
    double_num =
        my_f_str_to_num(*str, &shift, &isDigitExist, flags, &isWrongWidth);
...
void my_parse_spec(const char **str, const char **format, va_list *tr,
  ...
  int isSpec = (my_strchr("cs%%iduoxXpfn", **format) != my_NULL);
  switch (*(*format)++) {
    case 'c':
      *count +=
          my_handleCSpec(str, tr, &flags, isSpaceinfromat, read_from_str);
      break;
    case 's':
      *count += my_handleSSpec(tr, str, &flags, read_from_str);
      break;
    case '%':
      (*str)++;
      break;
    case 'i':
    case 'd':
      *count += my_handleDSpec(resWidth, str, tr, flags, read_from_str);
      break;
    case 'u':
      *count += my_handleUOSpec(resWidth, str, tr, flags, read_from_str, 10);
      break;
    case 'o':
      *count += my_handleUOSpec(resWidth, str, tr, flags, read_from_str, 8);
      break;
    case 'x':
    case 'X':
      *count += my_handleXSpec(resWidth, str, tr, flags, read_from_str);
      break;
    case 'p':
      *count += my_handlePSpec(resWidth, str, tr, flags, read_from_str);
      break;
    case 'f':
      *count += my_handleFSpec(resWidth, str, tr, flags, read_from_str);
      break;
    case 'n':
      my_handleNSpec(isSpec, str, tr, read_from_str);
      break;
    default:
      break;
  }
...
int my_handleSscanf(const char *str, const char *format, va_list *tr,
  ...
  while (*format != 0) {
    if (my_isspace(*format))
      isSpaceinfromat = my_handleSpacesInSpecifiers(&format, &lengthStr);
    ...
    if (*format++ == '%') {
      ...
      my_parse_flag_sscanf(format, &flags);
      ...
      my_parse_spec(&str, &format, tr, isSpaceinfromat, &read_from_str,
                     &resWidth, &count, flags);

      isWrongWidth = resWidth;
    }
    my_memset(&flags, 0, sizeof(flagss));
    flags.length_mode = 0;
  }
  return count;
}