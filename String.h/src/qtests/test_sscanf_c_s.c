...
START_TEST(test_parse_string) {
  ...
  // %c
  ck_assert_int_eq(my_sscanf("123", "%c", buf1), sscanf("123", "%c", buf2));
  ck_assert_int_eq(buf1[0], buf2[0]);

  // %s
  ck_assert_int_eq(my_sscanf("1234567890", "%s", buf1),
                   sscanf("1234567890", "%s", buf2));
  ck_assert_str_eq(buf1, buf2);

  // %s with len
  ck_assert_int_eq(my_sscanf("abcdefghk", "%5s", buf1),
                   sscanf("abcdefghk", "%5s", buf2));
  ck_assert_str_eq(buf1, buf2);

  // empty string
  ck_assert_int_eq(my_sscanf("", "%s", buf1), sscanf("", "%s", buf2));
  ck_assert_str_eq(buf1, buf2);

  // two strings with space
  char buf3[100] = {0}, buf4[100] = {0};
  ck_assert_int_eq(my_sscanf("123 456", "%s %s", buf1, buf3),
...
Suite *test_sscanf_c(void) {
  Suite *s = suite_create("\033[45m-=my_SSCANF_C_S=-\033[0m");
