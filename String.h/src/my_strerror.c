...
char *my_strerror(int errnum) {
  ...
  if (errnum >= MAX_ERRLIST || errnum <= MIN_ERRLIST) {
    ...
    part1[my_strlen(ERROR)] = '\0';
    my_errnum_tostring(part2, errnum);
...
void my_errnum_tostring(char *str, int num) {
  ...
  part[i] = '\0';
  for (my_size_t i = 0; i < my_strlen(part); i++) {
    if (flag == 0)
      str[i] = part[my_strlen(part) - i - 1];
    else
      str[i + 1] = part[my_strlen(part) - i - 1];
  }
  if (flag == 1) {
    str[0] = '-';
    str[my_strlen(part) + 1] = '\0';
  } else
    str[my_strlen(part)] = '\0';
}
