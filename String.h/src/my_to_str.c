...
void my_int_to_str(long int n, char s[], flag flags) {
  ...
  int rast = flags.accuracy - (int)my_strlen(s);
  while (rast > 0) {
    s[i++] = '0';
    rast--;
  }

  i = my_add_plus_space_minus(sign, s, flags, i);
...
void my_float_to_str(long double n, char s[], flag flags) {
  ...
  i = my_f_to_str(digit_part, s, decimal_places, i);

  s[i++] = '.';

  i = my_f_to_str(whole_part, s, num_digits, i);
...
void my_ftoa_forG(long double n, char s[], flag *flags) {
  ...
  int sign = 0;
  my_what_the_sign(&n, &sign);
  my_ftoa_forG_digits_roundn(&n, flags);
  my_ftoa_forG_add_all_parts(s, flags, n, &i);
  i = my_add_plus_minus(sign, s, *flags, i);
  my_reverse(s);
  s[i] = '\0';
}
...
void my_decimalToOctal(unsigned long int decimalNum, char octalStr[],
  ...
  int rast = flags.accuracy - (int)my_strlen(octalStr);
  while (rast > 0) {
    octalStr[i++] = '0';
    rast--;
  }
...
void my_decimalToHexdecimalSmallOrBig(unsigned long int decimalNum,
  ...
  // Дополнительные условия для обработки decimalNum = 0;
  my_additional_function_decimal(&decimalNum, &flags, hexStr, &i, mode);
  int rast = flags.accuracy - (int)my_strlen(hexStr);
  while (rast > 0) {
    hexStr[i++] = '0';
    rast--;
  }
...
void my_amount_of_acc_to_print_e(int *decimal_places, flag flags,
                                  int check_g) {
  if (flags.dot) {
    if (check_g) {
      if (flags.accuracy) {
...
void my_count_digits(long double *n, flag flags, int check_g,
...
int my_mantissa_to_str_e(char s[], int check_e, int num_of_digits,
...
int my_digits_to_str_e(unsigned long int digit_part, flag flags,
...
void my_e_float_to_str(long double n, char s[], flag flags, int check_e,
  ...
  my_what_the_sign(&n, &sign);
  my_count_digits(&n, flags, check_g, &num_of_digits, &mantissa_sign);
  if (!flags.dot) flags.accuracy = 6;
  ...
  my_amount_of_acc_to_print_e(&decimal_places, flags, check_g);
  i = my_mantissa_to_str_e(s, check_e, num_of_digits, mantissa_sign, i);
  i = my_digits_to_str_e(digit_part, flags, decimal_places, check_g, s, i, n);
